<template>
    <div id="save-main">
        <img src="../assets/static/savedata/18957.png" id="bg-savedata-image1">
        <img src="../assets/static/savedata/4772.png" id="bg-savedata-image2">
        <div>
            <div id="content-overlay">
                ...
                <div id="savedata-setting4">
                    <img src="../assets/static/savedata/10800.png">
                    <img src="../assets/static/savedata/10941.png" style="margin-left: 10%;">
                </div>
                <div class="data-container" @wheel="handleWheel">
                    <div id="data-grid" :style="{ transform: `translateY(-${PageValue * 23}vh)` }">
                        <div v-for="(item, index) in items" :key="item.id" class="data-block"
                            :style="{ cursor: (item.dataShow || (!twoClick && !isMode3or4)) ? 'pointer' : 'auto' }">
                            <img :src="item.isHover && (item.dataShow || (!twoClick && !isMode3or4)) ? item.hoverImage : item.defaultImage"
                ...
                <div id="savedata-setting3">
                    ...
                    <img :src="img(13)" class="setting-buttons3" @mouseover="handleMouseOver(13)"
                        @mouseout="handleMouseOut(13)" @mousedown="handleMouseDown(13)" @mouseup="handleMouseUp(13)">
...
//savedata-Text初始化
const saveMessage = ref('存档界面，请选择想要保存的进度。')
...
let mode = 0;//模式，0为保存，1为复制，2为替换，3为编辑，4为删除
const ensureClick = async (index) => {          //此处应该同时根据mode、选择的blockChoicedID1,blockChoicedID2进行判断。
    if (index === 0) {
        ...
        if (mode === 0) {
            console.log("执行保存操作");
            SaveItems.value[currentdata1.value.index].plot_id = GlobeTransmitItems.value[0].currentPlotId;
            SaveItems.value[currentdata1.value.index].background = GlobeTransmitItems.value[0].currentBackground;
            SaveItems.value[currentdata1.value.index].chapter = GlobeTransmitItems.value[0].currentChapter;
            SaveItems.value[currentdata1.value.index].storyArc = GlobeTransmitItems.value[0].currentStoryArc;
            SaveItems.value[currentdata1.value.index].comment = "COMMENT";
            SaveItems.value[currentdata1.value.index].timestamp = Date.now();
            try {
                console.log(currentdata1.value);
                const data = {
                    Did: currentdata1.value.Did, plot_id: GlobeTransmitItems.value[0].currentPlotId,
                    timestamp: Date.now(), comment: "COMMENT",
                    stroyArc: 0        //神人后端storyArc拼写错误打成stroyArc了，算了反正没用上就先这样，默认设0得了，能用就行。
                };
                const response = await postData(0, data);
        ...
        if (mode === 1) {
            ...
            try {
                ...
                const data = { Did1: currentdata2.value.Did, Did2: currentdata1.value.Did, timestamp: Date.now() };   //Did1传给Did2
...
const handleClick2 = (index) => {   //setting2一行
    ...
    if (mode === 3 || mode === 4) { isMode3or4.value = true; } else { isMode3or4.value = false; }
    const keys = Object.keys(clickStates.value);
    keys.forEach(key => {
        clickStates.value[key] = false;
    });//重新置为false
